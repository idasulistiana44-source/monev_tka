...
class TemplateReportController extends BaseController
{
    ...
    {
        // Hanya admin yang dapat mengambil data
        if (strtolower((string) session()->get('role')) !== 'admin') {
        ...
        try {
            ...
            log_message(
                'error',
                'Template Report getTemplate: ' . $e->getMessage()
            );
    ...
    {
        // Hanya admin yang dapat menyimpan template
        if (strtolower((string) session()->get('role')) !== 'admin') {
            return $this->response
                ->setStatusCode(403)
                ->setJSON([
        ...
        try {
            ...
            foreach ($json['sections'] as $section) {
                ...
                if (count($items) === 0) {
                    ...
                    if ($sectionId > 0) {
                        ...
                        $existing =
                            $this->templateReportModel->find($sectionId);

                        if ($existing) {

                            $this->templateReportModel->update(
                                $sectionId,
                                [
                            ...
                            $newId =
                                $this->templateReportModel->insert(
                                    [
                                        'section_title' => $sectionTitle,
                                        'item_title'    => '',
                                        'content'       => '',
                ...
                foreach ($items as $item) {
                    ...
                    $content = array_key_exists('content', $item)
                        ? (string) $item['content']
                        : null;
                    ...
                    if ($itemId > 0) {
                        ...
                        $existing =
                            $this->templateReportModel->find($itemId);
            ...
            // =====================================================
            // HAPUS DATA YANG SUDAH TIDAK ADA DI TEMPLATE
            // =====================================================
            $existingRows =
                $this->templateReportModel
                    ->select('id')
                    ->findAll();
            ...
            log_message(
                'error',
                'Template Report Save: ' . $e->getMessage()
            );
