$(document).ready(function(){
    ...
    function bindEvents(){
        ...
        $('#btnRefreshVisit').on('click',function(){
            loadVisits(true);
        });
        ...
        $('#visitAssignment').on('change',function(){
            showAssignmentInfo($(this).val());
        });
        $('#visitTableBody').on('click','.btn-start-visit',function(){
            startVisit($(this).data('id'));
        });
        $('#visitTableBody').on('click','.btn-continue-visit',function(){
            continueVisit($(this).data('id'));
        });
        $('#visitTableBody').on('click','.btn-view-visit',function(){
            viewVisit($(this).data('id'));
        });
    }
            ...
            success:function(response){
                ...
                if(response.success&&Array.isArray(response.data)&&response.data.length){
                    response.data.forEach(function(item){
                        select.append('<option value="'+item.id+'" data-school="'+escapeHtml(item.school_name||'')+'" data-npsn="'+escapeHtml(item.npsn||'')+'" data-level="'+escapeHtml(item.level||'')+'" data-officer="'+escapeHtml(item.officer_name||'')+'" data-date="'+escapeHtml(item.assignment_date||'')+'">'+escapeHtml(item.npsn||'-')+' - '+escapeHtml(item.school_name||'-')+' - '+escapeHtml(item.officer_name||'-')+'</option>');
                    });
                }else{
                    select.html('<option value="">Tidak ada assignment aktif</option>');
                    notify('Tidak ada assignment aktif yang dapat dibuatkan visitasi.','warning');
                }
            },
            error:function(xhr){
    ...
    function showAssignmentInfo(id){
        ...
        $('#visitAssignmentInfo').html('<div class="visit-assignment-info-grid"><div><small>Sekolah</small><strong>'+option.data('school')+'</strong></div><div><small>NPSN</small><strong>'+option.data('npsn')+'</strong></div><div><small>Level</small><strong>'+option.data('level')+'</strong></div><div><small>Petugas</small><strong>'+option.data('officer')+'</strong></div><div><small>Tanggal Assignment</small><strong>'+formatDate(option.data('date'))+'</strong></div></div>');
    }
    function saveVisit(){
    ...
    function getActionButton(item){
        if(item.status==='DRAFT'){
            return '<button type="button" class="btn btn-sm btn-primary btn-start-visit" data-id="'+item.id+'"><i class="fas fa-play me-1"></i>Mulai</button>';
        }
        if(item.status==='IN_PROGRESS'){
            return '<button type="button" class="btn btn-sm btn-primary btn-continue-visit" data-id="'+item.id+'"><i class="fas fa-arrow-right me-1"></i>Lanjutkan</button>';
        }
        return '<button type="button" class="btn btn-sm btn-outline-primary btn-view-visit" data-id="'+item.id+'"><i class="fas fa-eye me-1"></i>Lihat</button>';
    }
