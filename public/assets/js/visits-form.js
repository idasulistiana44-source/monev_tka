(function () {
    ...
    function ajax(url, options) {
        ...
        if (
            ...
            data[window.VISITS_FORM_CSRF_NAME] =
                window.VISITS_FORM_CSRF_HASH;
        }
    ...
    function loadForm() {
        ...
        $('#instrumentContainer').html(
            '<div class="visit-form-loading p-4 text-center">' +
                '<i class="fas fa-spinner fa-spin me-2"></i>' +
                'Memuat instrumen...' +
            '</div>'
        );

        ajax('visits/instruments/' + VISIT_ID)
            .done(function (res) {
    ...
    function renderSections() {
        ...
        if (!sections.length) {
            ...
            $('#instrumentContainer').html(
                '<div class="visit-no-instrument p-4 text-center">' +
                    '<i class="fas fa-clipboard-list fa-2x mb-2 text-muted"></i>' +
                    '<h5>Belum ada instrumen aktif</h5>' +
                    '<p class="text-muted">' +
                        'Instrumen visitasi belum tersedia.' +
                    '</p>' +
                '</div>'
            );
        ...
        $.each(sections, function (sectionIndex, section) {
            ...
            html +=
                '<div class="instrument-section mb-4">';

            html +=
                '<div class="instrument-section-header card-header bg-light mb-3 p-3">';

            html +=
                '<div class="d-flex align-items-center">';

            html +=
                '<div class="instrument-section-number badge bg-primary me-2 fs-6">' +
                    (sectionIndex + 1) +
                '</div>';
            ...
            html +=
                '<div class="instrument-section-body">';

            $.each(
                section.instruments || [],
                function (index, instrument) {
    ...
    if (String(item.code || '').trim().toUpperCase() === 'INF-15') {
        questionText = 'SEBUTKAN ISP CADANGAN YANG DIGUNAKAN';
    }

    let html =
        '<div class="instrument-item card mb-3 p-3" ' +
        'data-instrument="' + id + '" ' +
        'data-required="' + (isReq ? '1' : '0') + '">';

    html +=
        '<div class="instrument-question d-flex justify-content-between align-items-center mb-2">';

    html +=
        '<span class="instrument-code fw-bold">' +
        escapeHtml(item.code || '') +
        '</span>';

    if (isReq) {
        html +=
            '<span class="instrument-required text-danger fw-bold small">' +
            '*Wajib' +
            '</span>';
    }
    ...
    html +=
        '<div class="instrument-question-text mb-2">' +
        escapeHtml(questionText) +
        '</div>';

    if (item.description) {
        html +=
            '<div class="instrument-description text-muted small mb-2">' +
     ...
     *
     * INF-12:
     * Muncul hanya jika INF-11 = Lainnya
     *
     * INF-15:
     * Muncul hanya jika INF-14 = Lainnya
     *
     * INF-16:
     * Muncul jika INF-14 != Tidak Ada
     *
     * Field yang disembunyikan juga dikosongkan.
     */
    function applyISPConditional() {
    ...
    function getInstrumentCode(element) {
    ...
    function clearInstrumentValue(item) {
    ...
    html =
        '<div class="input-group">' +
            '<input type="number" class="form-control instrument-answer" ' +
                'data-id="' + id + '" ' +
        ...
        html =
            '<select class="form-select instrument-answer" ' +
            'data-id="' + id + '">' +
        ...
        html +=
            '<div class="mt-2 pdf-preview" style="display:none;">';
        ...
        html +=
            '<span class="pdf-name small text-muted"></span>';
        ...
        html +=
            ...
            'border-radius:8px;display:none;"></iframe>';
        ...
        html +=
            '<div class="mt-2 photo-preview" style="display:none;">';
    ...
    function bindAnswerEvents() {
        ...
        /*
         * EVENT CONDITIONAL ISP
         */
        $(document)
            .off(
                'change.ispConditional',
                '.instrument-answer'
            )
            .on(
                'change.ispConditional',
        ...
        $(document)
            .off(
                'change.instrumentFile',
                '.instrument-answer[type="file"]'
            )
            .on(
                'change.instrumentFile',
                '.instrument-answer[type="file"]',
    ...
    function updateStatus(status) {
        ...
        if (status === 'DRAFT') {
            ...
            statusEl.addClass(
                'status-draft badge bg-secondary'
            );
    ...
    function handleAjaxError(
        ...
        if (
            ...
            message =
                'Endpoint URL tidak ditemukan (404).';
    ...
    $(document).ready(function () {
        ...
        // BUKA MODAL SUBMIT
        $('#btnCompleteVisit, #btnSubmitVisits')
            .off('click')
            .on(
                'click',
                function (e) {
